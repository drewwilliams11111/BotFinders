...
const runBTCFinderBot = async () => {
    ...
    const collectionInfoCoins = await helperMongo.getCollectionInfo('BTCFinderSmall')
    ...
    for (let i = 0; i < allCoins.length; i++) {
        ...
        if (!allCoins[i].boughtOrNot && points >= 1 && allCoins[i].currentStage !== 0) {
            await buy(points)
        }
    ...
    await helperMongo.updateCollection('BTCFinderSmall', collectionInfoCoins[0]._id, 'allCoins', allCoins)
}
runBTCFinderBot()