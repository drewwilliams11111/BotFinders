...
const runBTCFinderBot = async () => {
    ...
    const collectionInfoCoins = await helperMongo.getCollectionInfo('BTCFinderChanging2LossSmall')
    ...
    for (let i = 0; i < allCoins.length; i++) {
        ...
        const getNewMovement = async () => {
            ...
            for (let x = 0; x < movement.length; x++) {
                if (movement[x].symbol === 'BTCUSDT') {
                    allCoins[i].movementBTC = movement[x].movementSmall
                }
        ...
        if (!allCoins[i].boughtOrNot && points >= 1 && allCoins[i].currentStage !== 0 && allCoins[i].normalBuying) {
            await buy(points)
        }
        if (!allCoins[i].boughtOrNot && points === 1 && !allCoins[i].normalBuying) {
            await buy(points)
        }
    ...
    await helperMongo.updateCollection('BTCFinderChanging2LossSmall', collectionInfoCoins[0]._id, 'allCoins', allCoins)
}
runBTCFinderBot()